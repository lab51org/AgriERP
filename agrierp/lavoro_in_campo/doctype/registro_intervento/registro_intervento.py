...
class Registro_intervento(Document):
	def validate(self):
	...
	def get_overlap_for(self, fieldname, args, value):
		...
		existing = frappe.db.sql("""select ts.name as name, tsd.from_time as from_time, tsd.to_time as to_time from
			`tabRegistro_intervento Detail` tsd, `tabRegistro_intervento` ts where {0}=%(val)s and tsd.parent = ts.name and
			(
...
@frappe.whitelist()
def get_projectwise_registro_intervento_data(project, parent=None):
	cond = ''
	if parent:
		cond = "and parent = %(parent)s"

	return frappe.db.sql("""select name, parent, billing_hours, billing_amount as billing_amt
			from `tabRegistro_intervento Detail` where parenttype = 'Registro_intervento' and docstatus=1 and project = %(project)s {0} and billable = 1
			and sales_invoice is null""".format(cond), {'project': project, 'parent': parent}, as_dict=1)

@frappe.whitelist()
def get_registro_intervento(doctype, txt, searchfield, start, page_len, filters):
	...
	return frappe.db.sql("""select distinct tsd.parent from `tabRegistro_intervento Detail` tsd,
			`tabRegistro_intervento` ts where
...
@frappe.whitelist()
def get_registro_intervento_data(name, project):
	data = None
	if project and project!='':
		data = get_projectwise_registro_intervento_data(project, name)
	else:
		data = frappe.get_all('Registro_intervento',
	...
	return {
		...
		'registro_intervento_detail': data[0].name if data and project and project!= '' else None
	}

@frappe.whitelist()
def make_sales_invoice(source_name, item_code=None, customer=None):
	target = frappe.new_doc("Sales Invoice")
	registro_intervento = frappe.get_doc('Registro_intervento', source_name)

	if not registro_intervento.total_billable_hours:
		frappe.throw(_("Invoice can't be made for zero billing hour"))

	if registro_intervento.total_billable_hours == registro_intervento.total_billed_hours:
		frappe.throw(_("Invoice already created for all billing hours"))

	hours = flt(registro_intervento.total_billable_hours) - flt(registro_intervento.total_billed_hours)
	billing_amount = flt(registro_intervento.total_billable_amount) - flt(registro_intervento.total_billed_amount)
	billing_rate = billing_amount / hours

	target.company = registro_intervento.company
	if customer:
	...
	target.append('registro_interventos', {
		'time_sheet': registro_intervento.name,
		'billing_hours': hours,
		'billing_amount': billing_amount
	})

	target.run_method("calculate_billing_amount_for_registro_intervento")
...
def set_missing_values(time_sheet, target):
	doc = frappe.get_doc('Registro_intervento', time_sheet)
	target.employee = doc.employee
	target.employee_name = doc.employee_name
	target.salary_slip_based_on_registro_intervento = 1
...
def get_events(start, end, filters=None):
	...
	conditions = get_event_conditions("Registro_intervento", filters)

	return frappe.db.sql("""select `tabRegistro_intervento Detail`.name as name,
			`tabRegistro_intervento Detail`.docstatus as status, `tabRegistro_intervento Detail`.parent as parent,
			from_time as start_date, hours, activity_type,
			`tabRegistro_intervento Detail`.project, to_time as end_date,
			CONCAT(`tabRegistro_intervento Detail`.parent, ' (', ROUND(hours,2),' hrs)') as title
		from `tabRegistro_intervento Detail`, `tabRegistro_intervento`
		where `tabRegistro_intervento Detail`.parent = `tabRegistro_intervento`.name
			and `tabRegistro_intervento`.docstatus < 2
			and (from_time <= %(end)s and to_time >= %(start)s) {conditions} {match_cond}
		""".format(conditions=conditions, match_cond = get_match_cond('Registro_intervento')),
		{
			"start": start,
			"end": end
		}, as_dict=True, update={"allDay": 0})

def get_registro_interventos_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="modified"):
	user = frappe.session.user
	# find customer name from contact.
	customer = ''
	registro_interventos = []
	...
	if customer:
		...
		# Return registro_intervento related data to web portal.
		registro_interventos = frappe.db.sql('''
			SELECT
			...
			FROM `tabRegistro_intervento` ts, `tabRegistro_intervento Detail` tsd
...
def get_list_context(context=None):
	return {
		...
		"show_search": True,
		'no_breadcrumbs': True,
		"title": _("Registro_interventos"),
		"get_list": get_registro_interventos_list,
		"row_template": "templates/includes/registro_intervento/registro_intervento_row.html"
	}
