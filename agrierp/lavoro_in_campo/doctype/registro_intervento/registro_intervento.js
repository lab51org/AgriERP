...
frappe.ui.form.on("Registro_intervento", {
	setup: function(frm) {
			...
			frm.add_custom_button(__(button), function() {
				...
				$.each(frm.doc.time_logs || [], function(i, row) {
					...
					if (flag && row.activity_type && !row.from_time){
						erpnext.registro_intervento.timer(frm, row);
					...
					if (flag && row.from_time <= frappe.datetime.now_datetime() && !row.completed) {
						...
						erpnext.registro_intervento.timer(frm, row, timestamp);
						flag = false;
					}
				});
				// If no activities found to start a timer, create new
				if (flag) {
					erpnext.registro_intervento.timer(frm);
				}
	...
	make_invoice: function(frm) {
		...
		dialog.set_primary_action(__('Create Sales Invoice'), () => {
			...
			return frappe.call({
				type: "GET",
				method: "erpnext.projects.doctype.registro_intervento.registro_intervento.make_sales_invoice",
				args: {
	...
	make_salary_slip: function(frm) {
		frappe.model.open_mapped_doc({
			method: "erpnext.projects.doctype.registro_intervento.registro_intervento.make_salary_slip",
			frm: frm
		});
	},
});

frappe.ui.form.on("Registro_intervento Detail", {
	...
	time_logs_add: function(frm) {
		...
		$trigger_again.on('click', () => {
			$('.form-grid')
				...
				.append(frappe.render_template("registro_intervento"));
	...
	activity_type: function(frm, cdt, cdn) {
		frappe.call({
			method: "erpnext.projects.doctype.registro_intervento.registro_intervento.get_activity_cost",
			args: {
